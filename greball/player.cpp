...
for(i=0 ; i<WALL ; i++)
{
if(x1<wall[i].le() && wall[i].le()<x2)
{
	yy=a*wall[i].le()+b;
	if(wall[i].top()<yy && yy<wall[i].bot())
	{
		if(cx==0)
			y=yy,x=wall[i].le()-15;
	}
}
if(x1<wall[i].ri() && wall[i].ri()<x2)
{
	yy=a*wall[i].ri()+b;
	if(wall[i].top()<yy && yy<wall[i].bot())
	{
...
if(y1<wall[i].top() && wall[i].top()<y2)
{
	xx=(wall[i].top()-b)/a;
	if(wall[i].le()<xx && xx<wall[i].ri())
	{
		if(cy==0)
			x=xx, y=wall[i].top()-15;
	}
}
if(y1<wall[i].bot() && wall[i].bot()<y2)
{
	xx=(wall[i].bot()-b)/a;
	if(wall[i].le()<xx && xx<wall[i].ri()){
			if(cy==1)
				x=xx, y=wall[i].bot()+15;
...
bool PLAYER::wantup(CWALL *wall, int WALL, bool right)
{
	int i;
		...
		for(i=WALL-1 ; i>=0 ; i--)
			if(AIUPWALL(wall[ai->bwall[i]],right))
				return 1;
	return 0;
}
...
int PLAYER::canup(CWALL *wall, int WALL, bool right, int x, int y)
{
	int i;
		...
		for(i=WALL-1 ; i>=0 ; i--)
			if(canwallup(wall[ai->bwall[i]],right,x,y))
				return ai->bwall[i];
	return -1;
}
...
{
front++;
que[front].set(x,y,x,wall[rr].bot()+radius,0,1,1);
front++;
que[front].halfset(x,y,x,wall[rr].bot()+radius,0,1,1);
}
rr=canup(wall,WALL,0,x,y);
if(rr != -1)
{
front++;
que[front].set(x,y,x,wall[rr].bot()+radius,0,0,1);
front++;
que[front].halfset(x,y,x,wall[rr].bot()+radius,0,0,1);
}
...
int ff=front;
	do{
		...
		if(front>head)
			...
			front++;
			que[front]=que[rear];
			que[front].nextset(que[rear].x,wall[rr].bot()+radius);
			front++;
			que[front]=que[rear];
			que[front].nextset((que[rear].x+que[rear].x)/2,(wall[rr].bot()+radius+que[rear].y)/2);
			}
